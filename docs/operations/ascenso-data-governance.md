...
Sentry replay integration is removed and replay/tracing sample rates are zero. Client/server/edge error hooks allow only event diagnostics, generic exception categories and sanitized stack filenames/line positions; arbitrary messages, request bodies, users, breadcrumbs, extra context and source-line contents are omitted. Credential query strings/fragments and scheduling capability path values are removed. This reduces diagnostic detail intentionally; support should use scoped operational history rather than transmitting private records in exception messages.
